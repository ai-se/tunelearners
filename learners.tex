...
To find our ``scout'',  we sorted  the above algorithms from simplest to most complex.
The three simplest optimizers are SA, $\mathcal{E}$-domination, and 
differential evolution (each can be coded in less than a page of some high-level scripting language). Our reading of the current literature is that there are more current advocated for
differential evolution than
  SA or $\mathcal{E}$-domination. when the MOEA/D community select a secondary optimizer, then often use  differential evolution~\cite{zhang07,5583335}.
  Also, Vesterstrom and Thomsen~\cite{Vesterstrom04} found that  differential evolution was competitive with 
